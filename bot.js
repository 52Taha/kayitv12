...
// Eğer Sunucuya Taglı Katılırsa İsmi Tag KÂMİL | 4 Eğer Sunucuya Taglı Olarak Katılmazsa KÂMİL | 4 Ve Taglı Rolü veriyor
...
client.on("userUpdate", async (stg, yeni) => {
  ...
  if (!(yeni.username).includes(ekipTag) && uye.roles.cache.has(ekipRolü)) {
    try {
      ...
      await uye.send(`Tagımızı bıraktığın için ekip rolün alındı! Tagımızı tekrar almak istersen;\nTagımız: **${ekipTag}**`);
      await client.channels.cache.get(logKanali).send(new Discord.MessageEmbed().setColor('RED').setDescription(`${yeni} adlı üye tagımızı bırakarak bize ihanet etti!`));
    } catch(err) { console.error(err) };
  };
});

//------------------------HOŞGELDİN-EMBEDLİ-----------------------\\     Mahşer Code   **BU KISIMI KENDİNİZE GÖRE DEĞİŞTİREBLİRSİNİZ**
    ...
    if(üs) {
      üyesayısı = üyesayısı.replace(/([0-9])/g, d => {
        return {
          ...
          '2': ``,
          '3': ``,
          '4': ``,     //HAREKETLİ VEYA NORMAL EMOJİLERİNİZİ <a:emojiadı:emoji idsi> şeklinde tırbakların arasına yazınız.
          '5': ``,
          '6': ``,
          '7': ``,
          '8': ``,
...
Kayıt edilmek için teyit odasına girip <@&{register}> yetkililerine teyit vermen yeterli !

Seninle birlikte `+üyesayısı+` kişi olduk!

Sunucumuzun kurallarına uymalısın aksi takdirde cezalandırılacaksın bu nedenle kurallarımızı okumanı tavsiye ederiz.

Sunucumuzun tagını (\`tagınız\`) alırsan seviniriz.

İçeride keyifli vakitler geçirmeni dileriz.`)
.setImage(`LİNK`) //Link yazan kısma herhangi bir resim linki yazınız gif ,jpeg ,png gibi türlerin hepsi desteklidir.
client.channels.cache.get(kanal).send(`<@&${register}>`)
client.channels.cache.get(kanal).send(embed)
});


//-----------------------GİRENE-ROL-VERME----------------------\\     Mahşer Code BU KISMA ELLEMENİZE GEREK YOK EĞER GİREN KİŞİYE ROL VERİLMESİNİ İSTEMİYORSANIZ SİLEBİLİRSİNİZ
